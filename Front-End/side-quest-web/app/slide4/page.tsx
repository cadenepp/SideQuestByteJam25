...
export default function Testing() {
  ...
  // Timer logic: resumes from localStorage and updates
  useEffect(() => {
    ...
    const fetchSlide = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/api/quest/slide4/${encodeURIComponent(sessionId)}`
        );
  ...
  // Handle choices
  const handleChoice = async (answer: number) => {
    ...
    // Successful path: send data to server
    try {
      const res = await fetch(
        `http://localhost:8080/api/SuccessfulUser/${userId}/${stoppedTime}/${sessionId}`,
        { method: "POST" }
      );
      if (!res.ok) throw new Error(`HTTP error! Status: ${res.status}`);
      console.log("Successful user data sent!");
    } catch (err) {
      console.error("Error sending successful user data:", err);
    } finally {
      localStorage.removeItem("elapsedTime");
